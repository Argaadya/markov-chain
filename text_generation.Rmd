...
## Requirement and Training Objective
...
>>>>>>> a985ae81e5359f7ee6e99762f479bc28396a9621
...
<<<<<<< HEAD
Based on the data, if current day is sunny,  there are 6 occurences that the next day would be cloudy, 18 occurences that the next day is rain, and 30 occurences that the next day is sunny again. We can directly calculate the probability. For example, if the current day is sunny, the probability that the next day would be sunny again is:

$$P(tomorrow = sunny\ |\ today = sunny) = \frac{30}{30+6+18} = 0.556$$

This probability is called as the **Transition Probability** and represent the probability of transition from sunny today to sunny again tomorrow. Below is the transition probability for each next state if today is sunny.
=======
...
You can also add laplace smoothing and slightly modify the equation:
...
The transition matrix is identical with the previous one. The next section will illustrate 
...
Supossed that we have the following transition matrix from a Health Insurance process:
=======
## Absorbing State
...
There are many application of Markov Chains in various field of industries. Before we proceed to the main topic of text generation, I will illustrate some use cases of Markov Chains in health insurance and manufacturing machine maintenance. 
...
```
```{r echo=FALSE}
fit_markov <- read_rds("sherlock.Rds")
```
...
```
```{r}
markov_bigram <- read_rds("bigram_sherlock.Rds")
```
=======
predictive_text <- function(text, num_word){
   text <- strsplit(text, " ") %>% unlist() %>% tail(1)
...
predictive_text <- function(text, num_word){
   ...
   markov_bigram$estimate[ tolower(text), ] %>%
   sort(decreasing = T) %>% 
   head(num_word) %>% 
   names() %>% 
...
```
```{r eval=FALSE}
markov_trigram <- read_rds("trigram_sherlock.Rds")
```
...
predictive_text <- function(text, num_word){
   ...
   markov_trigram$estimate[ tolower(text), ] %>%
   sort(decreasing = T) %>% 
   head(num_word) %>% 
   names() %>% 
   str_extract(pattern = "\\s(.*)") %>% 
...
predictive_text("i love my", num_word = 5)
```
```{r echo=FALSE}
read_rds("output_ngram.Rds")
```
=======
